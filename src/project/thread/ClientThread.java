...
public class ClientThread extends Thread
{
	...
	public void acceptMessage(String recMessage)
	{
		...
		{
			if (recMessage.startsWith("/endTurn"))
			{
				recMessage = recMessage.substring(8);
				//create game message alerting users of endturn
				String[] user = recMessage.split("::");
				gameclient.chatpad.chatLineArea.append(user[0] + "is finished with their turn \n");
				gameclient.chatpad.chatLineArea.setCaretPosition(
						gameclient.chatpad.chatLineArea.getText().length());
				if (recMessage.equals(GamePad.yourName))
				{
					gameclient.chatpad.chatLineArea.append("it is now the opponent's turn\n");
					gameclient.chatpad.chatLineArea.setCaretPosition(
							gameclient.chatpad.chatLineArea.getText().length());
					gameclient.gamepad.setEndOfTurn(true);
				}
				else
				{
					gameclient.chatpad.chatLineArea.append("it is now your turn\n");
					gameclient.chatpad.chatLineArea.setCaretPosition(
							gameclient.chatpad.chatLineArea.getText().length());
					gameclient.gamepad.setEndOfTurn(false);
				}
			}
			else if (recMessage.startsWith("/gameInfo"))
			{
				recMessage = recMessage.substring(9);
				String[] data = recMessage.split("!#@");
				String[] user = data[0].split("::");
				gameclient.chatpad.chatLineArea.append(user[0] + " " + data[1] +"\n");
				gameclient.chatpad.chatLineArea.setCaretPosition(
				gameclient.chatpad.chatLineArea.getText().length());
			}
			else
